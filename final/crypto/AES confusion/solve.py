# https://github.com/JorianWoltjer/Python-random-module-cracker/blob/master/randcrack/randcrack.py
class RandCrack:
    def __init__(self):
        self.counter = 0
        self.mt = []
...
def getrand():
    ...
    r.sendlineafter(b"Ciphertext: ",t)
    r.recvuntil(b"Plaintext: ")
    return bytes([j^0x10 for j in bytes.fromhex(r.recvline().decode())])

from tqdm import trange 

for i in trange(624//4):
    t = getrand()
    for i in range(0,16,4):
        rc.submit(int(t[i:i+4][::-1].hex(),16))

rc.offset(-624-2*4)
key = b"".join(pack(rc.predict_getrandbits(32)) for _ in range(4))
iv = b"".join(pack(rc.predict_getrandbits(32)) for _ in range(4))
